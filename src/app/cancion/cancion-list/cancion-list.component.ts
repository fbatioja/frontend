...
export class CancionListComponent implements OnInit {
  ...
  getCanciones():void{
    this.cancionService.getAllCanciones(this.userId, this.token)
    .subscribe(canciones => {
      this.canciones = canciones
      this.mostrarCanciones = canciones
      if(this.router.snapshot.params.cancionId === "-1"){
        this.onSelect(this.mostrarCanciones[0], 0)
      }
      else{
        this.onSelectById(this.mostrarCanciones, parseInt(this.router.snapshot.params.cancionId))
      }
    })
  }
  ...
  onSelectById(canciones: Array<Cancion>, id: number){
  ...
  eliminarCancion(){
    ...
    .subscribe(cancion => {
      this.ngOnInit()
      this.showSuccess()
      this.routerPath.navigate([`/canciones/${this.cancionSeleccionada.id}/${this.userId}/${this.token}`])
    },
    error=> {
      this.showError("Ha ocurrido un error. " + error.message)
    })
